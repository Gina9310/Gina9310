<p align="center">
  <img src="imagen_post_economia_circular_0.jpg" alt="Banner de perfil" width="100%" />
</p>

# 👋 Hola, soy Georgina

Soy **Ingeniera Ambiental** con **Maestría en Economía Circular** y experiencia en **análisis de datos** aplicados a sostenibilidad y marketing.  
Me apasiona transformar datos en estrategias que generen impacto positivo en empresas y en el medio ambiente.  

---

## 🚀 Habilidades principales
- **Lenguajes y herramientas**: Python (pandas, matplotlib, seaborn, statsmodels), SQL, GitHub, VS Code  
- **Data Analysis**: Exploratory Data Analysis (EDA), visualización de datos, modelos predictivos  
- **Machine Learning**: Logistic Regression, Random Forest, XGBoost  
- **Sostenibilidad**: Economía circular, gestión ambiental, cumplimiento normativo  
- **Soft Skills**: Comunicación clara, trabajo en equipo, orientación a resultados  

---

## 📊 Proyectos destacados
- [Análisis de ventas de videojuegos para plan de marketing](https://github.com/Gina9310/An-lisis-de-ventas-de-videojuegos-para-plan-de-marketing)  
- [Análisis de embudo de ventas para determinar comportamiento del usuario](https://github.com/Gina9310/An-lisis-de-embudo-de-ventas-para-determinar-el-comportamiento-del-usuario-a-cambio-en-la-aplicaci-n)
- [Análisis de RSU en México](https://github.com/Gina9310/An-lisis-RSU-en-M-xico](https://github.com/Gina9310/An-lisis-RSU-en-M-xico)

---
 
## 🌍 Conéctate conmigo
- [LinkedIn](https://www.linkedin.com/in/georgina-rodr%C3%ADguez/)  
- [Email](georginardz9310@gmail.com)
